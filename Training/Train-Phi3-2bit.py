...
try:
    ...
    # We'll use the same quantization method as was used for fine-tuning
    has_original_model = False
    
    try:
        # Try to load original model with the same quantization approach
        if USING_AQLM:
            print(f"Loading original model with AQLM {QUANT_BITS}-bit quantization...")
            ...
            # Now apply the same AQLM quantization
            try:
                if hasattr(aqlm, 'quantize'):
                    quantize_fn = aqlm.quantize
                elif hasattr(aqlm, 'quantization') and hasattr(aqlm.quantization, 'quantize'):
                ...
                # Apply quantization to the model
                original_model = quantize_fn(
                    original_base_model, 
                    bits=QUANT_BITS,
                    lora_rank=LORA_R
                )
                has_original_model = True
                print(f"Successfully loaded original model with AQLM {QUANT_BITS}-bit quantization")
                
            except Exception as e:
                print(f"Failed to apply AQLM quantization to original model: {e}")
                # Will try BitsAndBytes fallback
                original_model = None
                
        else:
            # If we're using BitsAndBytes quantization
            print("Loading original model with BitsAndBytes 4-bit quantization...")
            bnb_config = BitsAndBytesConfig(
                load_in_4bit=True,
                bnb_4bit_quant_type="nf4",
                bnb_4bit_compute_dtype=torch.float16,
            ...
            has_original_model = True
            print("Successfully loaded original model with BitsAndBytes 4-bit quantization")
    ...
    # For evaluation, use the model we already have loaded (test_model was set to model)
    fine_tuned_model_for_eval = model
    
    # Calculate perplexity for fine-tuned model
    fine_tuned_perplexity = calculate_perplexity(fine_tuned_model_for_eval, tokenized_val)
    ...
    def generate_response_for_eval(eval_model, prompt, max_tokens=200):
    ...
    for test in swift_benchmark:
        ...
        fine_tuned_response = generate_response_for_eval(fine_tuned_model_for_eval, test['prompt'])
        ...
        if has_original_model:
            ...
            original_response = generate_response_for_eval(original_model, test['prompt'])
    ...
    # 9. Save evaluation results - include quantization information
    evaluation_results = {
        "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
        "model_name": MODEL_NAME,
        "quantization": {
            "method": "AQLM" if USING_AQLM else "BitsAndBytes",
            "bits": QUANT_BITS
        },
    ...
    # Save to file
    evaluation_file = f"./phi3_swift_model/evaluation_results_{QUANT_BITS}bit_{evaluation_results['quantization']['method']}.json"
    ...
    for i, test in enumerate(detailed_examples):
        ...
        fine_tuned_response = generate_response_for_eval(fine_tuned_model_for_eval, test['prompt'])
        print(f"\nFine-tuned model ({QUANT_BITS}-bit {evaluation_results['quantization']['method']}) response:")
